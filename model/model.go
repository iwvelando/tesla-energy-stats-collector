...
func (r TegPowerwalls) ParseTime() error {
	for _, powerwall := range r.Powerwalls {
		for _, check := range powerwall.CommissioningDiagnostic.Checks {
		...
		for _, check := range powerwall.UpdateDiagnostic.Checks {
			if check.StartTimeRaw != "" {
				t, err := time.Parse(DateTimeNano, check.StartTimeRaw)
				if err != nil {
					return err
	...
	for _, check := range r.Sync.CommissioningDiagnostic.Checks {
		if check.StartTimeRaw != "" {
			t, err := time.Parse(DateTimeNano, check.StartTimeRaw)
			if err != nil {
				return err
	...
	for _, check := range r.Sync.UpdateDiagnostic.Checks {
		if check.StartTimeRaw != "" {
			t, err := time.Parse(DateTimeNano, check.StartTimeRaw)
			if err != nil {
				return err
